...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: ../../../public/seance/ajouter.php'); exit; }
$date     = $_POST['date'] ?? '';
...
if (!$date || strtotime($date) <= strtotime('today')) {
    header('Location: ../../../public/seance/ajouter.php?err='.urlencode('La date doit être au minimum demain.')); exit;
}
// CDC : 1 séance par salle par jour
if ((new SalleRepository())->estOccupee($refSalle, $date)) {
    header('Location: ../../../public/seance/ajouter.php?err='.urlencode('Cette salle est déjà occupée ce jour-là.')); exit;
}
