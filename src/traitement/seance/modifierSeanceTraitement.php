...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /Cine_Lumiere/public/seance/liste.php'); exit; }
$id       = (int)($_POST['id'] ?? 0);
$date     = $_POST['date'] ?? '';
...
if ($repo->aDesReservations($id)) { header('Location: /Cine_Lumiere/public/seance/liste.php?msg=reservee'); exit; }
// CDC : pas dans le passé ni le jour même
if (!$date || strtotime($date) <= strtotime('today')) {
    header('Location: /Cine_Lumiere/public/seance/modifier.php?id='.$id.'&err='.urlencode('La date doit être au minimum demain.')); exit;
}
// CDC : 1 séance par salle par jour (on exclut la séance actuelle)
if ((new SalleRepository())->estOccupee($refSalle, $date, $id)) {
    header('Location: /Cine_Lumiere/public/seance/modifier.php?id='.$id.'&err='.urlencode('Cette salle est déjà occupée ce jour-là.')); exit;
}
