...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: /Cine_Lumiere/public/reservation/liste.php'); exit; }
$id = (int)($_POST['id']??0);
$r  = (new ReservationRepository())->getById($id);
if (!$r || $r['etat']==='confirmée') { header('Location: /Cine_Lumiere/public/reservation/liste.php?msg=err'); exit; }
$a = (int)($_POST['nbre_places_adulte']??0);
$s = (int)($_POST['nbre_places_senior']??0);
$e = (int)($_POST['nbre_places_etudiant']??0);
if (($a+$s+$e) < 1) { header('Location: /Cine_Lumiere/public/reservation/modifier.php?id='.$id.'&err='.urlencode('Au moins 1 place requise.')); exit; }
